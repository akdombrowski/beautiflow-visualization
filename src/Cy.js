...
export const createElementsObjFromArrays = (nodesArray, edgesArray) => ({nodes: nodesArray, edges: edgesArray});
...
export const getCopyOfAnnotationsArr = flowJSON => {
	try {
		...
		return Array.filter(nodes, (n, i, nodes) => n.data.nodeType === 'ANNOTATION');
	} catch (error) {
...
export const convertToBasicNodesAndEdgesAndCreateCytoscapeGraph = flowJSON => {
...
export const isEleYPosWithinToleranceOfY = (ele, y, tolerance) => ele.position.y < y + tolerance && ele.position.y > y - tolerance;

export const getNodesWithinYTolerance = (cy, y, tolerance) => cy.filter((ele, i, eles) => {
	if (!ele.isNode()) {
		return false;
	}
...
export const getCyNodesInRowAtYPos = (cyNodes, y, tolerance) => cyNodes.filter((ele, i, eles) => {
...
export const animateNodesAndWait = async (cy, nodes, dur, color) => Promise.all(
...
export const animateElesAndWait = async (cy, els, dur, color) => Promise.all(
	els.map((ele, i, eles) => {
		const message = 'animated ' + ele.id();
		return new Promise((resolve, reject) => {
			let sty;
			sty = ele.isNode() ? {backgroundColor: color} : {'line-color': color};
...
export const doesNodePathMergeWithAlreadyVisitedNodes = (
	...
	const {value, ele} = removeUnvisitedNodes.max((ele, i, eles) => ele.position('y'));
           ...
           */
             ...
             */
                ...
                = previousOutgoerNodesSorted.toArray();
							for (const [j, element] of previousOutgoerNodesSortedArray.entries()) {
     ...
     */
		...
		if (watchAnimation) {
			...
			for (const ani of animations) {
				...
				if (vMovePos) {
					message = 'animating ' + ani.currID + ' to ' + JSON.stringify(vMovePos);
					const resolvedMessage = await emitAndWaitForAni(
						message,
						currID,
						cy,
				...
				if (preID) {
					...
					const {ani: doneWithPreviousNodeAni, prom} = getAnimationPromiseForEle(
						cy.$('#' + preID),
						dur,
						'green',
					);
